...
X_OFFSET = 100
POS_TEXTO = [(550, 50)]
POS_BOTOES = [(550, 300), (550, 370), (550, 440), (550, 510)]
TAM_BOTAO = (150, 50)
TAM_TEXTO = (400, 100)
...
class TelaPause(AbstractTela):
    ...
    def run(self):
        ...
        menu_loop = True
        while menu_loop:
            clock.tick(self.__MENUPAUSE_FPS)
            self.__desenhar()
    ...
    def __criar_textos(self):
        branco = (255, 255, 255)
        self.__textos.append(Texto(POS_TEXTO[0], TAM_TEXTO, 'Pause', 60, branco))

    def __criar_botoes(self):
        funcVoltarJogo = self.__comandos[ComandosEnum.TELA_JOGAR]
        funcSalvarJogo = self.__comandos[ComandosEnum.LOAD_GAME]
        funcTelaOpcoes = self.__comandos[ComandosEnum.TELA_OPCOES]
        funcVoltarMenu = self.__comandos[ComandosEnum.TELA_MENU]
        
        self.__botoes.append(Botao(POS_BOTOES[0], TAM_BOTAO, 'Voltar', funcVoltarJogo ))
        self.__botoes.append(Botao(POS_BOTOES[1], TAM_BOTAO, 'Save', funcSalvarJogo))
        self.__botoes.append(Botao(POS_BOTOES[2], TAM_BOTAO, 'Opções', funcTelaOpcoes))
        self.__botoes.append(Botao(POS_BOTOES[3], TAM_BOTAO, 'Menu', funcVoltarMenu)) #arrumar warning libpng
